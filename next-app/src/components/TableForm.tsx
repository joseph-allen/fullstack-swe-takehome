...
const TableForm: React.FC<TableFormProps> = ({ onSubmit, isLoading }) => {
  const {
    register,
    handleSubmit,
  ...
  const renderPartyWarning = () =>
    size >= MAX_TABLE_SIZE ? (
      <Alert severity="warning">
        <Typography variant="body2">
          For parties larger than {MAX_TABLE_SIZE}, please speak to the staff.
        </Typography>
      </Alert>
    ) : null;

  const renderPartyImages = () => (
    <Box
      minHeight="128px"
      display="flex"
      flexWrap="wrap"
  ...
  return (
    <Box sx={{ width: 400, maxWidth: 400, p: 3 }}>
      <form onSubmit={handleSubmit(internalSubmit)} noValidate>
        <Stack spacing={3}>
          ...
          <Box>{renderPartySizeControls()}</Box>
